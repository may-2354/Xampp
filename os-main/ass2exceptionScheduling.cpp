...
void validateInput(int processId, int arrivalTime, int burstTime) {
    if (processId < 0) {
        throw invalid_argument("Process ID should not be negative.");
    }
    if (arrivalTime < 0) {
        throw invalid_argument("Arrival time should not be negative.");
    }
    if (burstTime <= 0) {
        throw invalid_argument("Burst time should be greater than 0.");
    }
    if (arrivalTime >= burstTime) {
        throw invalid_argument("Burst time should be greater than arrival time.");
    }
}
...
int main() {
    ...
    cout << "Enter the process IDs (positive integers only): \n";
    for (int i = 0; i < n; i++) {
        while (true) {
            cin >> process[i];
            if (process[i] < 0) {
                cout << "Invalid Process ID. Please enter a non-negative value: ";
            } else {
                break;
            }
        }
    ...
    cout << "Enter the arrival time for the respective processes (>= 0): \n";
    for (int i = 0; i < n; i++) {
        while (true) {
            cin >> at[i];
            if (at[i] < 0) {
                cout << "Invalid Arrival Time. Please enter a non-negative value: ";
            } else {
                break;
            }
        }
    ...
    cout << "Enter the burst time for the respective processes (>= 1): \n";
    for (int i = 0; i < n; i++) {
        while (true) {
            cin >> bt[i];
            if (bt[i] < 1) {
                cout << "Invalid Burst Time. Please enter a value >= 1: ";
            } else {
                break;
            }
        }
        ...
        switch (choice) {
            ...
            case 4: {
                ...
                while (true) {
                    cin >> timeQuantum;
                    if (timeQuantum < 1) {
                        cout << "Invalid Time Quantum. Please enter a value >= 1: ";
                    } else {
                        break;
                    }
                }
            ...
            case 5:
                ...
                for (int i = 0; i < n; i++) {
                    while (true) {
                        cin >> priority[i];
                        if (priority[i] < 0) {
                            cout << "Invalid Priority. Please enter a non-negative value: ";
                        } else {
                            break;
                        }
                    }
